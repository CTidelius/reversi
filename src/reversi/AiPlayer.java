...
public class AiPlayer extends ReversiPlayer {
	...
	public void makeMove(ReversiGame board, int ai_color) {
		...
		if(legal_actions.size() == 0) return;
	...
	private int calculateBranchScore(ReversiGame simulated_game,
			boolean is_maximizer_turn, int ai_color, long done_by, long time_limit) {
		...
		if (System.nanoTime() >= done_by - 12000) {
			...
			if(legal_actions.size() == 0){
				...
				for (Point tile : legal_actions) {
					...
					int branch_value = calculateBranchScore(game_copy, !is_maximizer_turn,
							ai_color, done_by, time_limit);
					done_by += time_limit;
